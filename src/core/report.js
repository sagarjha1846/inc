...
/**
 * Neutralise a string for use as plain prose inside a Markdown document.
 *
 * `brand` and `prepared-for` are typically set by whoever runs the audit, but
 ...
 * dashboard where a client's own display name becomes `brand`, at which point
 * it is no longer trusted. Left raw, `<script>…</script>` rides straight
 * through into the document — GitHub and most renderers pass inline HTML in
 * Markdown through unescaped — and `[Renew your key](https://evil.example)`
...
export function renderMarkdown(result, options = {}) {
  ...
  if (result.issuesWithheld > 0) {
    lines.push(`> ${result.issuesWithheld} further finding(s) and all generated fix files are included in ${mdText(brand)} Pro.`);
    lines.push('');
  }
  ...
  lines.push('---');
  lines.push('');
  lines.push(`Generated by ${mdText(brand)}.`);
  lines.push('');
  return lines.join('\n');
}
    ...
    if (rollup.sitewideIssuesWithheld > 0) {
      ...
      lines.push(
        `> Showing ${rollup.sitewideIssues.length} of ${rollup.sitewideIssuesTotal} template-level issues. ` +
          `${rollup.sitewideIssuesWithheld} more are included in ${mdText(brand)} Pro.`,
      );
      lines.push('');
    }
  }
